...
/**
 * Injects the dataSource in the
 * {@link javax.persistence.spi.PersistenceUnitInfo} according to the chosen
 * mode. By default, JTA is not enabled and the dataSource is injected as a
 * non-jta aware data source. If the {@link #setJtaEnabled(boolean)} flag is set
 * to <tt>true</tt> the dataSource is injected as a jta-aware data source.
 */
public class JtaPersistenceUnitPostProcessor implements PersistenceUnitPostProcessor {
	 ...
	 * the <tt>dataSource</tt> as a jta data source if it is <tt>true</tt> or as
	 * a regular, non-jta data source otherwise.
	 *
	 ...
	 *
	 * @param jtaEnabled
	 *            <tt>true</tt> to enable jta support
	 */
