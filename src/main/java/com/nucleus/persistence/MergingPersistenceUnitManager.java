 ...
 * 
 * Refer this blog entry for more details:
 * http://labs.bsb.com/2010/11/configuring-modular-jpa-applications-with-spring/
 * <br/>
 * The solution presented in this blog is different, more complex and little
 * different than what we require. Therefore it has been customized to suit our
 * needs.
 * 
 * @author praveen.jain
 */
public class MergingPersistenceUnitManager extends DefaultPersistenceUnitManager {
	...
	protected void postProcessPersistenceUnitInfo(MutablePersistenceUnitInfo pui) {
		...
		// If this is our base persistence unit that we are using to merge all
		// others.. we will skip its processing
	...
	/**
	 * Sets the name of the persistence unit that should be used for merging. If
	 * no such persistence unit exists, an exception will be thrown, preventing
	 * the factory to be created.
	 * 
	 * @param persistenceUnitName
	 *            the name of the persistence unit to be used for merging
	 */
	...
	protected void mergePersistenceUnit(MutablePersistenceUnitInfo from, MutablePersistenceUnitInfo to) {
		...
		if (from.getManagedClassNames().size() != 0) { // Either take into
														// account the listed
														// entities
			for (String s : from.getManagedClassNames()) {
				to.addManagedClassName(s);
			}
			logger.debug("Added [" + from.getManagedClassNames().size() + "] managed classes from PU [" + fromName
					+ "] to " + "PU [" + toName + "]");
		} else { // Or work with classpath scan
			/*
			 * The base PU's classpath will be by default scanned for entities.
			 * Therefore if 'from' PU's root URL is different from base PU's
			 * root URL, only then add the 'from' PU's URL into base PU's URL
			 * list
			 */
	...
	/**
	 * Returns the base {@link MutablePersistenceUnitInfo} to use for merging,
	 * based on the given {@link MutablePersistenceUnitInfo} and the settings of
	 * the manager.
	 * 
	...
	private MutablePersistenceUnitInfo getMainPersistenceUnitInfo(MutablePersistenceUnitInfo pui) {
		...
		// Sanity check
		if (result == null) {
			throw new IllegalStateException("No persistence unit found with name [" + basePersistenceUnitName + "] "
